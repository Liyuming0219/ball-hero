...
class WaveManager {
    ...
    update(dt, playerX, playerY, enemies, particles) {
        ...
        if (this.spawnTimer >= config.spawnRate && enemies.length < ENEMY_CAP) {
            this.spawnTimer = 0;
            let count = config.count + Math.floor(this.gameTime / 150);
...
class ExpGem {
    ...
    update(dt, playerX, playerY, pickupRange, particles) {
        ...
        if (this.attracted) {
            ...
            if (distSq < 225) { // 15^2
                this.alive = false;
                // 拾取效果（简化：仅2粒子，无glow）
                particles.emit(this.x, this.y, 2, {
    ...
    render(ctx, camera, screenW, screenH) {
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(sx - r * 0.2, sy - r * 0.35, r * 0.2, 0, TWO_PI);
        ctx.fill();
...
class EnemyBullet {
    ...
    render(ctx, camera) {
        ...
        ctx.fillStyle = this.color;
        const trailCount = Math.min(len, 3);
        const trailStart = len - trailCount;
        for (let i = trailStart; i < len; i++) {
            const slot = ((start + i) % 6) * 2;
            const tx = buf[slot] - camera.x;
            const ty = buf[slot + 1] - camera.y;
            ctx.globalAlpha = (i / len) * 0.25;
            ctx.beginPath();
            ctx.arc(tx, ty, this.radius * (0.4 + 0.6 * i / len), 0, TWO_PI);
            ctx.fill();
        }
        // 弹体（合并脉动到主体）
        ctx.globalAlpha = 1;
        ...
        ctx.globalAlpha = 0.7;
        ctx.beginPath();
        ctx.arc(sx, sy, this.radius * 0.4, 0, TWO_PI);
        ctx.fill();
        ctx.globalAlpha = 1;
    }
}
